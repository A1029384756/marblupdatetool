...
class ESPLoader {
    ...
    constructor(transport, baudrate) {
    ...
    write_flash = async ({fileArray=[], flash_size='keep', flash_mode='keep', flash_freq='keep', erase_all=false, compress=true} = {}) => {
        ...
        for (var i = 0; i < fileArray.length; i++) {
            ...
            let calcmd5 = MD5(enc.Latin1.parse(image));
            ...
            while (image.length > 0) {
                ...
                if (compress) {
                    /*
                    let block_uncompressed = inflate(block).length;
                    ...
                    if (this.timeout_per_mb(this.ERASE_WRITE_TIMEOUT_PER_MB, block_uncompressed) > 3000) {
                        ...
                        block_timeout = 3000;
                    }*/ // XXX: Partial block inflate seems to be unsupported in  Hardcoding timeout
